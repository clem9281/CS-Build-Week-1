...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        return JsonResponse({'name':player.user.username, 'title':nextRoom.title, 'description':nextRoom.description, 'players':players, 'error_msg':"", 'inventory':player.inventory,'room_items': room.items}, safe=True )
    else:
        players = room.playerNames(player_id)
